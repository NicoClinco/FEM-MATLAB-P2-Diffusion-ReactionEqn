function [A,b_E,Ad,u_d] = Matrice_Stabilizzata(BC,forzante,beta,TheMesh)
...
Nquad=length(OMEGA);
[phi,gradphi,HessPhi]=mapping.map(N3,N1,N2,OMEGA,6);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for e=1:geom.nelements.nTriangles
    ...
    M=inv(B)*inv(B'); %%matrice di rototraslazione
    for j=1:6
        ...
        if(jj>0)
            for k=1:6
                ...
                if(kk>0)
                    ...
                    %Stabilizzazione di Brezzi-Pit
                    DiffStab=0; ConvStab=0; ReactStab=0;
                    for nq=1:Nquad
                        %Termine di diffusione per stabilizzare:
                        DiffStab=DiffStab+2*LocalArea*OMEGA(nq)*epsilon*(M(1,1)*HessPhi(1,1,k)+(M(1,2)+M(2,1))*HessPhi(1,2,k)+M(2,2)*HessPhi(2,2,k))*...
                            (beta')*inv(B')*[gradphi(j).x(nq);gradphi(j).y(nq)]*Tau;
                        %Termine di convezione per stabilizzare:
                        ConvStab=ConvStab+2*LocalArea*OMEGA(nq)*Tau*((beta')*inv(B')*[gradphi(k).x(nq);gradphi(k).y(nq)])*...
                            ((beta')*inv(B')*[gradphi(j).x(nq);gradphi(j).y(nq)]);
                    end

                    A(jj,kk)=A(jj,kk)+diff+conv+react-DiffStab+ConvStab;
                    ...
                    DiffStab1=0; ConvStab1=0; ReactStab1=0;
                    for nq=1:Nquad
                        %Termine di diffusione per stabilizzare:
                        DiffStab1=DiffStab1+2*LocalArea*OMEGA(nq)*epsilon*(M(1,1)*HessPhi(1,1,k)+(M(1,2)+M(2,1))*HessPhi(1,2,k)+M(2,2)*HessPhi(2,2,k))*...
                            (beta')*inv(B')*[gradphi(j).x(nq);gradphi(j).y(nq)]*Tau;
                        %Termine di convezione per stabilizzare:
                        ConvStab1=ConvStab1+2*LocalArea*OMEGA(nq)*Tau*((beta')*inv(B')*[gradphi(k).x(nq);gradphi(k).y(nq)])*...
                            ((beta')*inv(B')*[gradphi(j).x(nq);gradphi(j).y(nq)]);
                    end

                    Ad(jj,-kk)=Ad(jj,-kk)+diff1+conv1+react1+DiffStab1+ConvStab1;
            ...
            %Stabilizzazione della forzante
            fS=0;
            for nq=1:Nquad
                fS=fS+OMEGA(nq)*2*LocalArea*forzante(mapping.FE(V3,B,N1(nq),N2(nq)),beta)*((beta')*inv(B')*[gradphi(j).x(nq);gradphi(j).y(nq)])*Tau;
            end
            b_E(jj,1)=b_E(jj,1)+f+fS;
        end %if jj>0
